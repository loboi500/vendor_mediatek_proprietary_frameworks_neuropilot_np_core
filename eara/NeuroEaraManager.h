...
typedef struct EaraTimer {
    ...
    inline bool isValid() { return (jobStartTime_ > -1); }
...
public:
    ...
    int notifyExecutionBegin_2_1(int tid, uint64_t modelId, vector<int32_t>& boostValue,
    ...
    int notifyExecutionEnd_2_1(int tid, uint64_t modelId, vector<bool>& hasFallback,
                               vector<uint64_t>& execTime, vector<int32_t>& bandwidth,
    ...
    int getAverageExecutionTime(vector<vector<uint64_t>>& averageExecTime);
