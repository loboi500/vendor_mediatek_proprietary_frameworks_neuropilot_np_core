...
bool isEaraQosSupported() {
    if (sEaraSupport != -1) {
#ifdef NN_DEBUGGABLE
        if (property_get_bool("debug.nn.earaqos.verbose", /*default_value*/ false)) {
            VLOG << "isEaraQosSupported: " << sEaraSupport;
        }
#endif
    ...
    dlclose(handle);
#ifdef NN_DEBUGGABLE
    char var[PROPERTY_VALUE_MAX] = {0x00};
...
int ANeuroEara_create(ANeuroEara** eara, vector<string>& plan) {
    ...
    if (!eara) {
        LOG(ERROR) << "ANeuroEara_create passed a nullptr";
...
int ANeuroEara_setExecutionPreference(ANeuroEara* eara, ExecPreference preference) {
    if (!isEaraQosSupported()) {
        return ANEUROEARA_NOT_SUPPORT;
    }
    if (!eara) {
...
int ANeuroEara_getEaraUsage(ANeuroEara* eara, string comp, int* deviceUsage, int* bandwidth) {
    if (!isEaraQosSupported()) {
        return ANEUROEARA_NOT_SUPPORT;
    }
    if (!eara) {
...
int ANeuroEara_configSuspendParams(ANeuroEara* eara, string comp, int usageThreshold,
...
int ANeuroEara_notifyExecutionBegin(ANeuroEara* eara, int tid, uint64_t modelId,
...
int ANeuroEara_notifyExecutionFail(ANeuroEara* eara, int tid, uint64_t modelId, uint32_t stepId) {
    if (!isEaraQosSupported()) {
        return ANEUROEARA_NOT_SUPPORT;
    }
    if (!eara) {
...
int ANeuroEara_getAverageExecutionTime(ANeuroEara* eara,
                                       vector<vector<uint64_t>>& averageExecTime) {
    if (!isEaraQosSupported()) {
        return ANEUROEARA_NOT_SUPPORT;
    }
    if (!eara) {
        LOG(ERROR) << "ANeuroEara_getAverageExecutionTime passed a nullptr";
        return ANEUROEARA_UNEXPECTED_NULL;
    }
    NeuroEaraManager* e = reinterpret_cast<NeuroEaraManager*>(eara);
    return e->getAverageExecutionTime(averageExecTime);
}
