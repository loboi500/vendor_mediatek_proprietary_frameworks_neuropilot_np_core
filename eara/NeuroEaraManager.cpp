...
#define VLOG_EARA()                \
    if (!isEaraVerboseSupported()) \
        ;                          \
    else                           \
        LOG(INFO)

static void* getEaraLibHandle() {
...
static bool isEaraVerboseSupported() {
    static const bool supported =
        android::base::GetBoolProperty("debug.neuron.runtime.AnnVerbose", false);
    return supported;
}

// TODO: vector<vector<int32_t>> planStruct can change to pass by reference
typedef int (*earaNotifyJobBegin_fn)(int tid, uint64_t modelId, vector<vector<int32_t>> planStruct,
                                     vector<vector<uint64_t>>* suggestExecTime);

// TODO: STLs planStruct, nanoSec,  bandwidth, boostValue can change to pass by reference
typedef int (*earaNotifyJobEnd_fn)(int tid, uint64_t modelId, int32_t* jobPriority, int execSuccess,
                                   vector<vector<int32_t>> planStruct,
                                   vector<vector<uint64_t>> nanoSec,
                                   vector<vector<int32_t>> bandwidth,
...
inline static DeviceComponent compStringToEnum(string& comp) {
    if (s_DeviceComponentHash.count(comp)) {
        ...
        VLOG_EARA() << "[compStringToEnum] invalid component:" << comp;
        return DeviceComponent::DEVICE_COMP_UNKNOWN;
    }
    VLOG_EARA() << "[compStringToEnum] map " << comp << " -> " << s_DeviceComponentHash[comp];
    return s_DeviceComponentHash[comp];
}

NeuroEaraManager::NeuroEaraManager(vector<string>& plan) {
    mModelTag = " (MTag_" + std::to_string(reinterpret_cast<uint64_t>(this)) + ")";
    VLOG_EARA() << "Construct" << mModelTag << " - contains " << plan.size() << " steps";
...
void NeuroEaraManager::setCompilation(vector<string>& plan) {
    ...
    for (unsigned int i = 0; i < plan.size(); i++) {
        VLOG_EARA() << "[setCompilation] step[" << i << "] exec by [" << plan[i] << "]" << mModelTag;
...
int NeuroEaraManager::setExecutionPreference(ExecPreference preference) {
    VLOG_EARA() << "[setExecutionPreference] mPreference = " << preference << mModelTag;
    mPreference = preference;
    return ANEUROEARA_NO_ERROR;
}

int NeuroEaraManager::configSuspendParams(string& compStr, int usageThreshold,
                                          uint32_t sleepTimeInMs) {
    ...
    VLOG_EARA() << "[configSuspendParams] " << componentToString(comp)
         << " usageThreshold: " << usageThreshold << ", sleepTimeInMs: " << sleepTimeInMs
         << mModelTag;
...
int NeuroEaraManager::suggestSuspendTime(string& compStr, uint32_t* time) {
    ...
    if (bandwidth > BANDWIDTH_BOUND_THRESHOLD) {
        VLOG_EARA() << "[suggestSuspendTime] bandwidth bound! bandwidth:" << bandwidth << mModelTag;
        *time = static_cast<uint32_t>(BANDWIDTH_BOUND_SLEEP_TIME_MS);
    } else if (deviceLoading > mUsageThreshold[comp]) {
        VLOG_EARA() << "[suggestSuspendTime] " << componentToString(comp)
             << " bound! bandwidth:" << bandwidth << mModelTag;
...
int NeuroEaraManager::getEaraUsage(DeviceComponent comp, int* deviceLoading, int* bandwidth) {
    ...
    VLOG_EARA() << "[getEaraUsage] " << componentToString(comp) << " usage:" << (*deviceLoading);
    return ANEUROEARA_NO_ERROR;
}
...
int NeuroEaraManager::suggestOffload(string compStr, bool* execOffloadPlan) {
    ...
    if (mJobPriority == JOB_PRIORITY_LOW) {
        int deviceUsage = 0;
        int bandwidth = 0;
        NeuroEaraResultCode status =
            static_cast<NeuroEaraResultCode>(getEaraUsage(compStr, &deviceUsage, &bandwidth));
        if (status != ANEUROEARA_NO_ERROR) {
            return status;
        }
        if (deviceUsage > mUsageThreshold[compStringToEnum(compStr)]) {
            (*execOffloadPlan) = true;
            VLOG_EARA() << "[suggestOffload] bandwidth:" << bandwidth << ", deviceUsage:" << deviceUsage
                 << mModelTag;
        }
    }
    return ANEUROEARA_NO_ERROR;
...
int NeuroEaraManager::notifyExecutionBegin(int tid, uint64_t modelId,
    ...
    VLOG_EARA() << "[notifyExecutionBegin] => tid:" << tid << ", mid:" << modelId << mModelTag;
...
NeuroEaraResultCode NeuroEaraManager::notifyEaraJobBegin(int tid, uint64_t modelId,
                                                         vector<int32_t>& boostValue) {
...
NeuroEaraResultCode NeuroEaraManager::updateBoostValue(vector<int32_t>& boostValue) {
    ...
    for (unsigned int stepId = 0; stepId < mPlanStructs.size(); stepId++) {
        if (mBoostValue[stepId][0] == 0 || mSuggestExecTime[stepId][0] == INVALID_EXEC_TIME ||
            ...
            mBoostValue[stepId][0] =
                std::max(mModelMinExecTime[stepId][0] * 100 / mSuggestExecTime[stepId][0],
                         static_cast<uint64_t>(MIN_BOOST_VALUE));
        }
        boostValue[stepId] = mBoostValue[stepId][0];
        VLOG_EARA() << "[updateBoostValue] step" << stepId << " @"
             ...
             << ", boostValue: " << boostValue[stepId] << mModelTag;
    }
    return ANEUROEARA_NO_ERROR;
}

int NeuroEaraManager::notifyExecutionEnd(int tid, uint64_t modelId, vector<bool>& hasFallback,
    ...
    for (unsigned int stepId = 0; stepId < mPlanStructs.size(); stepId++) {
        ...
        VLOG_EARA() << "[notifyExecutionEnd] step" << stepId << " @"
                    << componentToString(static_cast<DeviceComponent>(mPlanStructs[stepId][0]))
...
int NeuroEaraManager::notifyExecutionEnd_2_1(int tid, uint64_t modelId, vector<bool>& hasFallback,
                                             vector<uint64_t>& execTime, vector<int32_t>& bandwidth,
                                             vector<int32_t>& boostValue) {
...
NeuroEaraResultCode NeuroEaraManager::notifyEaraJobEnd(int tid, uint64_t modelId,
    ...
    jobEnd_fn(tid, modelId, &mJobPriority, execSuccess ? 0 : -1, mPlanStructs, mModelExecTime,
              mBandwidth, mBoostValue);
    VLOG_EARA() << "[notifyEaraJobEnd] <= priority: " << mJobPriority << ", tid:" << tid
         << ", mid:" << modelId << mModelTag << '#';
    return ANEUROEARA_NO_ERROR;
}

int NeuroEaraManager::notifyExecutionFail(int tid, uint64_t modelId, uint32_t stepId) {
    VLOG_EARA() << "[notifyExecutionFail]" << mModelTag << " fail at step " << stepId;
...
int NeuroEaraManager::getAverageExecutionTime(vector<vector<uint64_t>>& averageExecTime) {
    averageExecTime = mModelMinExecTime;
    return ANEUROEARA_NO_ERROR;
}
