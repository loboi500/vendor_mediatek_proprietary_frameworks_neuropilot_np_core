...
static void* sNpAgentServerLibHandle;

inline void* loadNpAgentLibrary(const char* name) {
    sNpAgentServerLibHandle = dlopen(name, RTLD_LAZY | RTLD_LOCAL);

    if (sNpAgentServerLibHandle == nullptr) {
        char* error = nullptr;
        ...
        NPAGENT_LOG_D("NPAGENT : open NpAgent library %s", name);
    }
    return sNpAgentServerLibHandle;
}

inline void* getNpAgentLibraryHandle() {
    if (sNpAgentServerLibHandle == nullptr) {
        sNpAgentServerLibHandle = loadNpAgentLibrary("libnpagent_server.so");
    }
    return sNpAgentServerLibHandle;
}
    ...
    if (getNpAgentLibraryHandle() != nullptr) {
        ...
        if (fn == nullptr) {
            if ((error = dlerror()) != NULL) {
                NPAGENT_LOG_E("unable to open NpAgent service function %s, with error %s", name,
                              error);
                return nullptr;
            }
        }
...
typedef bool (*NpAgentServerDeInit_fn)();
