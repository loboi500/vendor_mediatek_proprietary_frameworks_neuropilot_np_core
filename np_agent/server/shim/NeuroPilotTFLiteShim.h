...
typedef int (*ANeuroPilotTFLiteOptions_setExecParallel_fn)(ANeuralNetworksTFLiteOptions* options,
                                                           bool enableParallel);
...
typedef int (*ANeuroPilotTFLite_setExecParallel_fn)(ANeuralNetworksTFLite* tflite,
                                                    bool enableParallel);
...
inline void* getTFLiteLibraryHandle() {
    ...
    if (sdk_version < 30) {
        ...
        if (sTFLiteHandle == nullptr) {
            // Load library for APK JNI level development
            sTFLiteHandle = loadTFLiteLibrary("libtflite_mtk_static_R.so");
        }
    }
    return sTFLiteHandle;
}
