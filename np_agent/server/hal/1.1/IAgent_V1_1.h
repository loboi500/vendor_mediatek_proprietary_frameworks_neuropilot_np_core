...
class IAgent_V1_1 : public IAgent {
public:
    ...
    Return<int32_t> prepareModelFromCache_1_1(const hidl_memory& cache,
        ...
        return mImpl->prepareModelFromCache(cache, params);
    }

    Return<void> getParams(int32_t modelId, getParams_cb _hidl_cb) override {
        auto params = mImpl->getParams(modelId);
        _hidl_cb(ErrorStatus::NONE, params);
        return Void();
    }

protected:
    std::unique_ptr<IAgentImpl_V1_1> mImpl = std::make_unique<IAgentImpl_V1_1>();
